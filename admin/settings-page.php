...
function course_certificate_admin_certificate_ui() {
	...
	if( isset($_POST['add_certificate']) ) {
		...
		} else if( empty($_POST['certificate_code']) || empty($_POST['std_name']) || empty($_POST['course_name']) || empty($_POST['course_hours']) || empty($_POST['dob']) ) {
        ...
        if(!wp_verify_nonce($_POST['course_nonce'], 'admin_certificate_ui')) {
            echo '<div class="alert alert-danger">Bulk Upload: Verification Failed!</div>';
        } else {
            ...
            if ($handle !== false) {
                ...
                while (($data = fgetcsv($handle, 1000, ',')) !== false) {
                    // New CSV order: first name, course, hours, certificate no, date of completion
                    if ($row == 0 && strtolower($data[0]) == 'first name') { $row++; continue; }
                    ...
                    course_certificate_add_course_certificate($code, $name, $course, $hours, $dob, '');
                    $row++;
                }
                fclose($handle);
                $error = '<div class="alert alert-success hide-alert">Bulk upload completed!<button type="button" class="close" data-dismiss="alert">x</button></div>';
            } else {
                $error = '<div class="alert alert-danger hide-alert">Bulk upload failed to read file!<button type="button" class="close" data-dismiss="alert">x</button></div>';
            }
        }
    }
	global $wpdb;
	    ...
	    <div class="table-title">
	      <div class="row">
	        ...
	        <div class="col-sm-6">
	          ...
	          <a href="#bulkUploadModal" class="btn btn-primary" data-toggle="modal"><i class="material-icons">&#xE2C6;</i> <span>Bulk Upload</span></a>
	        </div>
	      </div>
	    </div>
	    <?php echo $error;?>
	      ...
	      <tbody>
	        <?php foreach ($certificatesNew as $value) { ?>
	    	 	<tr>
	                ...
	                <td class="cadt" date="<?php echo $value->dob; ?>"><?php echo date("d/M/Y", strtotime($value->dob)); ?></td>
			        <td>
	      ...
	      <form class="mt-40" method="POST" enctype="multipart/form-data">
	        ...
	        <div class="modal-body">
	          <div class="form-group">
	            ...
	            <input type="file" class="form-control" name="bulk_certificate_csv" accept=".csv" required>
	            <small>CSV columns: certificate_code, student_name, course_name, course_hours, dob</small>
	          </div>
	        </div>
	        <div class="modal-footer">
	          <input type="button" class="btn btn-default" data-dismiss="modal" value="Cancel">
	...
	<script type="text/javascript">
		jQuery(document).ready(function() {
			...
			jQuery(document).on("click", ".editModal", function() {
				...
				jQuery("#editEmployeeModal input[name=dob]").val(ocadt);
				jQuery("#editEmployeeModal #editdob").val(cadt);
